...
export default function OrderDetail() {
  ...
  const isDelivery = result?.fulfillmentType === "DELIVERY" || deliveryFee > 0;
          ...
          <div className="pt-2 pb-4">
            {result.items.map((item, inx) => (
              ...
              >
                ...
                {(item.menuItem?.discountType === "BOGO" ||
                  item.menuItem?.discountType === "BOGOHO") &&
                  item.menuItem?.bogoItems?.length > 0 && (
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        <h5 className="font-medium text-green-700">
                          {item.menuItem.discountType === "BOGO"
                            ? "Free Items (Buy One Get One)"
                            : "Half Price Items (Buy One Get One Half Off)"}
                        </h5>
                      </div>
                      {item?.menuItem?.bogoItems.map((bogoItem, bogoInx) => (
                        <div
                          key={`bogoItem-${inx}-${bogoInx}`}
                            ...
                            <div className="flex gap-8 items-center">
                              <div className="text-center">
                                ...
                                <div className="font-medium text-green-600">
                                  {item?.menuItem?.discountType === "BOGO"
                                    ? "$0.00"
                                    : `$${Number(bogoItem?.halfPrice || bogoItem.price / 2).toFixed(2)}`}
                                </div>
                              </div>
                              <div className="text-center">
                                <div className="text-sm text-gray-500">Qty</div>
                              ...
                              <div className="text-center min-w-[80px]">
                                ...
                                <div className="font-medium text-green-600">
                                  {item?.menuItem?.discountType === "BOGO"
                                    ? "$0.00"
                                    : `$${Number(bogoItem?.total || (bogoItem?.halfPrice || bogoItem?.price / 2) * bogoItem.qty).toFixed(2)}`}
                                </div>
                              </div>
                            </div>
                          </div>
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        {result.refundDateTime && (
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium">
                              {dayjs(result.refundDateTime).format(
                                "DD MMM, YYYY hh:mm A",
                              )}
                            </span>
                          </div>
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {money(result.tax ?? result.taxAmount)}
                      </span>
                    </div>
                    {isDelivery && (
                      <>
                    ...
                    {result.paymentProcessingFee > 0 && (
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                          {money(result.paymentProcessingFee)}
                        </span>
                      </div>
                    )}
